...
def index(request):
    ...
    context = {
        ...
        'mimes': mimes,
        'exts': '|'.join(exts),
        # The ASAF frontend uses the same REST API key generated by this
        # MobSF installation. The key is injected at render time and is
        # never stored in the GitHub source as a literal secret.
...
class Upload(object):
    ...
    def upload(self):
        ...
        elif self.file_type.is_exe():
            return scanning.scan_exe()
        else:
            return {
                'description': 'File format not supported by the ASAF scanner.',
                'status': 'error',
            }
