...
from mobsf.StaticAnalyzer.views.android.priority_ranking import (
    get_priority_findings,
)
from mobsf.StaticAnalyzer.views.android.permission_chart import (
    get_permission_chart_data,
)
from mobsf.StaticAnalyzer.views.android.scan_trends import get_scan_trends
...
def get_context_from_db_entry(db_entry: QuerySet) -> dict:
    ...
    try:
        ...
        context = {
            ...
            'network_security': python_dict(
                db_entry[0].NETWORK_SECURITY),
            'binary_analysis': python_list(
                db_entry[0].BINARY_ANALYSIS),
            'file_analysis': python_list(
                db_entry[0].FILE_ANALYSIS),
            'android_api': python_dict(
                db_entry[0].ANDROID_API),
            'code_analysis': code,
            'niap_analysis': python_dict(
                db_entry[0].NIAP_ANALYSIS),
            'permission_mapping': python_dict(
                db_entry[0].PERMISSION_MAPPING),
            ...
            'firebase_urls': python_list(
                db_entry[0].FIREBASE_URLS),
            'files': python_list(db_entry[0].FILES),
            'exported_count': python_dict(
                db_entry[0].EXPORTED_COUNT),
            'apkid': python_dict(db_entry[0].APKID),
            'behaviour': python_dict(db_entry[0].QUARK),
            'trackers': python_dict(db_entry[0].TRACKERS),
            'playstore_details': python_dict(
                db_entry[0].PLAYSTORE_DETAILS),
        ...
        try:
            ...
            logger.exception(
                'SentinelX risk score calculation failed.')
            context['risk_score'] = {
                'score': 0,
                'grade': 'N/A',
            }
        ...
        # ---------------------------------------------------------
        # SentinelX Priority Findings
        # ---------------------------------------------------------
        try:
            context['priority_findings'] = get_priority_findings(
                context)
        except Exception:
            logger.exception(
                'SentinelX priority findings calculation failed.')
            context['priority_findings'] = []

        # ---------------------------------------------------------
        # SentinelX Permission Chart
        # ---------------------------------------------------------
        try:
            context['permission_chart'] = get_permission_chart_data(
                context)
        except Exception:
            logger.exception(
                'SentinelX permission chart calculation failed.')
            context['permission_chart'] = {}

        # ---------------------------------------------------------
        # SentinelX Scan Trends
        # IMPORTANT: get_scan_trends expects the complete context.
        # ---------------------------------------------------------
        try:
            context['scan_trends'] = get_scan_trends(context)
        except Exception:
            logger.exception(
                'SentinelX scan trends calculation failed.')
            context['scan_trends'] = {
                'trends': [],
            }
    ...
    try:
        ...
        context = {
            ...
            'browsable_activities': (
                man_an_dic['browsable_activities']),
            ...
            'malware_permissions': (
                man_an_dic['malware_permissions']),
            'manifest_analysis': manifest_analysis,
...
def save_get_ctx(
        ...
        rscn,
        user=None):
    """Save analysis data and return the analysis context."""
